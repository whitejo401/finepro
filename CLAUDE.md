...
## 세션 재개 규칙
**새 세션 시작 시**: `STATUS.md`를 가장 먼저 읽는다. 이 파일 하나로 현재 단계·미완료 작업·미커밋 파일을 파악한다. PLANNING.md는 상세 스펙이 필요할 때만 읽는다.

## STATUS.md 자동 갱신 규칙
아래 상황이 발생하면 즉시 `STATUS.md`를 갱신한다. 별도 확인 없이 자동 처리.

| 트리거 | 갱신 항목 |
|--------|---------|
| API 그룹 구현 완료 | 해당 그룹 상태 `🔨 구현중` → `✅ 완료` |
| 새 API 그룹 구현 시작 | 상태 `⬜ 미시작` → `🔨 구현중`, 위치 경로 기재 |
| 커밋 실행 | 미커밋 변경사항 섹션에서 해당 항목 제거 |
| 새 파일/폴더 생성 | 미커밋 변경사항 섹션에 추가 |
| 다음 할 일 완료 | 해당 항목 제거 또는 완료 표시 |
| 새 작업 계획 확정 | 다음 할 일 섹션에 추가 |
| 새 API 키 필요 확인 | 추가 필요 API 키 섹션에 추가 |
| 마지막 작업일 | 세션 종료 시 날짜 갱신 |
